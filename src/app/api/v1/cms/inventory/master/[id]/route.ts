...
export async function GET(
	...
	try {
		...
		const data = await getMasterInventoryByIdCached(id);
...
const getMasterInventoryByIdCached = (id: string) =>
	unstable_cache(
		async () => {
			// console.log(`🚀 Ambil dari DB untuk ID ${id}, belum cache`);
			return await useServices.GetById(id);
		},
		["master-inventory-cache", id], // cache key unik per id
		{ revalidate: 1000, tags: ["master-inventory-cache"] }
	)();
